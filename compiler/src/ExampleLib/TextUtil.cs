...
public static class TextUtil
{
    ...
    public static RgbColor ParseRgbColor(string text)
    {
        ValidateColor(text);

        string hexColor = text.Substring(1);
        byte red, green, blue;

        switch (hexColor.Length)
        {
            case 3:
                red = ParseHex($"{hexColor[0]}");
                green = ParseHex($"{hexColor[1]}");
                blue = ParseHex($"{hexColor[2]}");
                break;
            case 6:
                red = ParseHex(hexColor.Substring(0, 2));
                green = ParseHex(hexColor.Substring(2, 2));
