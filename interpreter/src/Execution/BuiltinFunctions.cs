...
public static class BuiltinFunctions
{
    ...
    public static bool TryInvoke(string name, List<Value> args, out decimal result)
    {
        if (Functions.TryGetValue(name, out Func<List<decimal>, decimal>? func))
        {
            List<decimal> decimals = args.Select(v => v.AsFloat()).Select(i => (decimal)i).ToList();
            result = func(decimals);
            return true;
        }
